...
<div class="container mt-4">
  ...
  <?php else:
    ...
    $stmt->bind_param('i',$batchId); $stmt->execute(); $stmt->bind_result($assignmentId);
    if (!$stmt->fetch()) { echo '<div class="alert alert-warning">No assignment for this batch.</div>'; include 'footer.php'; exit; }
    $stmt->close();
        // check submission exists
    $stmt = $mysqli->prepare("SELECT doc_path,pdf_path,text_content FROM afsm_assignment_submissions WHERE assignment_id=? AND student_id=?");
    $stmt->bind_param('ii',$assignmentId,$studentId);
    $stmt->execute();
    $stmt->bind_result($docPath, $pdfPath, $textContent);
    if (!$stmt->fetch()) {
    ...
    echo '<div class="mb-4">';
    echo '<h5>Submission Details</h5>';
    if ($docPath) {
        echo '<p>DOC File: <a href="'.htmlspecialchars($docPath).'" target="_blank">'.htmlspecialchars(basename($docPath)).'</a></p>';
    }
    if ($pdfPath) {
        echo '<p>PDF File: <a href="'.htmlspecialchars($pdfPath).'" target="_blank">'.htmlspecialchars(basename($pdfPath)).'</a></p>';
    }
    if (trim($textContent) !== '') {
        echo '<div><strong>Text Submission:</strong><div class="border p-2 mt-1">'.nl2br(htmlspecialchars($textContent)).'</div></div>';
    }
    echo '</div>';
    ...
    $stmt->bind_param('i',$batchId); $stmt->execute();
    ...
    if ($rubricItems) {
        ...
        $stmt->bind_param('ii',$assignmentId,$studentId); $stmt->execute();
      ...
      <div class="table-responsive">
        <table class="table table-bordered align-middle">
          <thead class="table-light">
            <tr><th>No</th><th>Criteria</th><th>1-Fair</th><th>2-Good</th><th>3-Excellent</th><th>Score</th></tr>
          </thead>
          <tbody>
            <?php foreach ($rubricItems as $i=>$item): ?>
              <tr>
                ...
                <td>
                  <input type="number" name="scores[<?= $item['id'] ?>]" min="0" max="<?= $item['max_score'] ?>" required class="form-control" value="<?= htmlspecialchars($existing[$item['id']] ?? '') ?>" oninvalid="this.setCustomValidity('Please enter a score between 0 and <?= $item['max_score'] ?>')" oninput="this.setCustomValidity('')">
                </td>
              </tr>
            <?php endforeach; ?>
          </tbody>
